...
def test_output(output, *args) -> None:
    ...
    assert output['aisles'].isnull().sum() == 0, 'Todavía hay valores nulos en la columna aisle'
    assert (output['aisles'] == 'missing').sum() == 0, "Todavía hay filas con 'missing' en la columna aisle"
